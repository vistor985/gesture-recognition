...
class DragRect():
    ...
    def update(self, cursor,setin):
        cx, cy, = self.posCenter[0:2]
        w, h = self.size
...
def gestureMouse(img,rectList):
    setin = [0]
    img = cv2.flip(img, 1)  # 翻转相机
    ...
    if hands:
        ...
        if l < 30:
            ...
            for rect in rectList:
                rect.update(cursor,setin)
                if setin[0]==1:
                    break
    # 透明绘制
    imgNew = np.zeros_like(img, np.uint8)
    for rect in rectList:
        cx, cy = rect.posCenter[0:2]
        w, h = rect.size
